...
with st.expander("❓ Glosario / FAQ — ¿Qué significa cada variable?"):
    st.markdown(
        "#### Datos SAP2000 y direcciones\n"
        "| Variable | Significado |\n"
        "|---|---|\n"
        "| **Steady-State** | Análisis de estado estacionario de SAP2000: barre frecuencias y entrega la respuesta forzada en régimen permanente. De ahí se obtiene la FRF. |\n"
        "| **U1 / U2 / U3** | Componentes de desplazamiento en los ejes globales SAP (1, 2, 3). U3 suele ser el vertical. |\n"
        "| **StepType** | Tipo de dato por frecuencia: *Mag* (magnitud), *Real* e *Imag* (partes real/imaginaria, usadas para la fase). |\n"
        "| **Dir** | Dirección evaluada del nodo: **Z** = vertical; en caso contrario horizontal/lateral. Selecciona los valores verticales u horizontales de K_din, f_rd, F_op y F_rd. |\n"
        "\n"
        "#### Frecuencias\n"
        "| Variable | Significado |\n"
        "|---|---|\n"
        ...
        "| **f_rd** | Frecuencia del **modo de cuerpo rígido** del equipo sobre el aislador: f_rd = (1/2π)·√(K_din/m). Es la frecuencia que el equipo **atraviesa al partir o detenerse** (condición transitoria). Se calcula por dirección: **f_rd_V** (vertical, con K_din_V) y **f_rd_L** (lateral, con K_din_L). En modo *Manual* la otorga el fabricante del aislador. |\n"
        ...
        "| **A** | **Amplitud** de desplazamiento peak: A = FRF · F / F_ref (mm). Aquí **F** es la fuerza dinámica de la condición evaluada (ver sección *Fuerzas*) y **F_ref** la carga unitaria con que se normalizó la FRF en SAP2000. |\n"
        ...
        "| Variable | Significado |\n"
        "|---|---|\n"
        "| **F** | Fuerza dinámica aplicada en A = FRF·F/F_ref. **Depende de la condición**: ① operación → **F_op**; ② transitorio → **F_rd** o F(f) según el modelo; ③ peak → **F(f_peak)**. |\n"
        "| **F(f)** | Función fuerza–frecuencia (modelo seleccionable en el sidebar). *Valor fijo*: F(f) = F_rd **constante**. *Curva desbalance*: F(f) = m·e·ω² (**∝ f²**), con ω = 2·π·f. *Fabricante F∝1/f*: F(f) = F_op·(f_op/f), fuerza **inversa a la frecuencia** (sólo condición ③ peaks). |\n"
        ...
        "| **F(f_peak)** | Fuerza dinámica de la condición ③, evaluada en la frecuencia del peak estructural. *Valor fijo* = F_op; *Curva* = m·e·(2π·f_peak)²/N_apoyos; *Fabricante F∝1/f* = F_op·(f_op/f_peak). |\n"
        ...
        "| **K_din** | Rigidez **dinámica** del aislador (N/mm). Puede definirse por dirección: **K_din_V** (vertical) y **K_din_L** (lateral). |\n"
        ...
        "| Variable | Significado |\n"
        "|---|---|\n"
        "| **Cortocircuito vibratorio** | Ocurre cuando la estructura es demasiado flexible y el aislador **no logra desacoplarla** de la máquina. Se descarta con el criterio **K_est/K_din ≥ 10** (ACI 351.3R-04, §3.3); el caso gobernante es el de mayor FRF_op. |\n"
        "| **Transmisibilidad (T)** | Fracción de la fuerza dinámica que **cruza el aislador** hacia la estructura: T = √(1+(2ξr)²) / √((1−r²)²+(2ξr)²), con r = f_op/f_res. **T < 1** indica aislamiento efectivo. |\n"
        "| **ξ (amortiguamiento)** | Razón de amortiguamiento del sistema (adimensional), usada en la transmisibilidad. |\n"
        "| **r = f_op/f_res** | Razón entre la frecuencia de operación y la de resonancia. La zona de aislamiento es r > √2. |\n"
        "| **Distorsión angular (Δφ)** | Diferencia de fase **entre apoyos** de una misma máquina a la frecuencia de evaluación. Un Δφ elevado impone **torsión dinámica** cíclica sobre el chasis/estructura. |\n"
        "\n"
        ...
        "- **En zona / Diagnóstico** (columnas de la condición ③): indican si el peak estructural cae **dentro** de la zona de exclusión y si corresponde a una **resonancia real** (fase ≈ 90°)."
    )
...
with tab_class:
    ...
    if not resultados:
        ...
        if modo_fuerza_tr.startswith("Curva"):
            ...
            st.caption(f"Peak estructural de |H| dentro de la zona de exclusión "
                       f"[{f_excl_lo:.1f}–{f_excl_hi:.1f} Hz], excitado por la fuerza **evaluada en "
                       "f_peak**. En modo **valor fijo** la función F(f) es constante, por lo que "
        ...
        with st.expander("ℹ️ Cómo se calculan las amplitudes (3 condiciones)"):
            st.markdown(
                ...
                "en modo **valor fijo** es constante (= F_op); en modo **Fabricante F∝1/f** "
                "$F(f_{peak})=F_{op}\\cdot(f_{op}/f_{peak})$ (inversa a la frecuencia: mayor a menor "
                "RPM, por la alarma de velocidad de vibración constante del fabricante). Responde "
                "*qué tan severa sería la "
                ...
                "**Cobertura de datos — qué pasa si f_rd cae fuera del barrido SAP:**\n"
                "- **f_rd por DEBAJO del rango** (típico: f_rd≈7 Hz, datos 30–80 Hz): el peor caso se "
                "busca solo donde **hay datos** (ventana ∩ barrido), así que la frecuencia más baja "
                "que se usa es el **mínimo del barrido SAP** (p.ej. 30 Hz), no f_rd. En modo curva, "
                "la fuerza de desbalance $F(f)$ se evalúa desde esa f mínima hacia arriba. El punto "
                "`A(f_rd)` se marca **NaN** porque no hay FRF en f_rd (no se extrapola).\n"
                "- **f_rd DENTRO del rango:** `A(f_rd)` se evalúa en f_rd. La FRF se toma en f_rd y "
                "la fuerza según el modelo: **valor fijo** → F_rd constante (F rd V/H del "
                "fabricante); **curva** → $F(f_{rd})=m e\\,(2\\pi f_{rd})^2/N_{apoyos}$.\n"
                "- Si la ventana queda **totalmente fuera** del barrido, se cae al punto más cercano "
                "(ver aviso amarillo). Todo es **por nodo y caso**: cada fila tiene su propio peor caso."
            )
